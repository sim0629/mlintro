...
\begin{align*}
    C &= \{ \mathbf{c}^{(i)} \}_{i=1}^{1600} ( \forall i, \mathbf{c}^{(i)} \in [0, 1]^{6 \times 6 \times 3} )
\end{align*}
...
\begin{align*}
    ...
    f_{i}'(\mathbf{p}^{*}) &= || \mathbf{p}^{*} - \mathbf{c}^{(i)} ||^2
\end{align*}
...
\begin{figure}[!b]
\includegraphics[width=\textwidth]{feature_extraction}
\centering
\caption{전체적인 feature 추출 과정 (Coates et al., 2011)}
\end{figure}
